...
// Mock implementation for Toy Theater Icon
const drawToyTheaterRecommendation = (doc: jsPDF, data: any, y: number) => {
    // Colors
    const colors: Record<string, [number, number, number]> = {
        'Math': [59, 130, 246], // Blue
        'Reading': [239, 68, 68], // Red
        'Art': [245, 158, 11], // Yellow/Orange
        'Music': [168, 85, 247], // Purple
        'Puzzles': [16, 185, 129], // Green
        'Games': [14, 165, 233], // Sky
        'default': [107, 114, 128] // Gray
    };

    const color = colors[data.category] || colors['default'];

    // Check space
    if (y + 50 > PAGE.height - PAGE.margin) {
        doc.addPage();
        y = PAGE.margin;
    }
    ...
    // Header Band (simulated with clipping/rect)
    doc.setFillColor(color[0], color[1], color[2]);
    // Draw top part
    doc.roundedRect(PAGE.margin, y, PAGE.contentWidth, 8, 3, 3, 'F');
    // Fix bottom corners of header
    doc.rect(PAGE.margin, y + 4, PAGE.contentWidth, 4, 'F');

    // Title
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(9);
    doc.setFont('helvetica', 'bold');
    doc.text(`RECOMENDAÇÃO TOY THEATER: ${data.category ? data.category.toUpperCase() : 'GAME'}`, PAGE.margin + 4, y + 5.5);

    // Icon Placeholder (Circle with initial)
    doc.setFillColor(255, 255, 255);
    doc.circle(PAGE.margin + 12, y + 20, 8, 'F');
    doc.setTextColor(color[0], color[1], color[2]);
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text(data.category ? data.category[0] : 'T', PAGE.margin + 9, y + 22.5);

    // Content
    doc.setTextColor(COLORS.text[0], COLORS.text[1], COLORS.text[2]);
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.text(data.name || 'Jogo Educativo', PAGE.margin + 25, y + 16);

    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(COLORS.textLight[0], COLORS.textLight[1], COLORS.textLight[2]);
    const descLines = doc.splitTextToSize(data.description || '', PAGE.contentWidth - 30);
    doc.text(descLines, PAGE.margin + 25, y + 21);

    // Link
    const linkY = y + 21 + (descLines.length * 4);
    doc.setTextColor(color[0], color[1], color[2]);
    doc.setFontSize(9);
    doc.setFont('helvetica', 'bold');
    doc.textWithLink('CLIQUE PARA ACESSAR O JOGO', PAGE.margin + 25, linkY, { url: data.url || 'https://toytheater.com' });
...
export const exportMarkdownToPDF = (title: string, markdown: string, filename: string) => {
    ...
    // Extract Toy Theater JSON Logic
    let toyTheaterData = null;
    let cleanMarkdown = markdown;
    const jsonMatch = markdown.match(/---TOY_THEATER_DATA---\s*([\s\S]*?)\s*---TOY_THEATER_DATA---/);

    if (jsonMatch) {
        try {
            toyTheaterData = JSON.parse(jsonMatch[1]);
            cleanMarkdown = markdown.replace(jsonMatch[0], '').trim();
        } catch (e) {
            console.error("Error parsing Toy Theater JSON", e);
        }
    }

    const lines = splitTextWithBold(internalDoc, cleanMarkdown, PAGE.contentWidth);
    lines.forEach((line: string) => {
    ...
    // Render Toy Theater Box at the end if exists
    if (toyTheaterData) {
        y = drawToyTheaterRecommendation(internalDoc, toyTheaterData, y);
    }
